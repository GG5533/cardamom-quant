...
## Corpus Check
- 84 files · ~189,408 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 861 nodes · 1626 edges · 74 communities (65 shown, 9 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 246 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `88d52bea`
...
- `test_fx_features_ffill_limited()` --calls--> `parse_fred_csv()`  [INFERRED]
  tests/test_signal_layer.py → /Users/samihabbal/Documents/Claude/Projects/My personal Profile and who i am/cardamom-quant/src/data/fx.py
- `_basis_market()` --calls--> `build_market_dataset()`  [INFERRED]
  tests/test_signal_layer.py → /Users/samihabbal/Documents/Claude/Projects/My personal Profile and who i am/cardamom-quant/src/data/loaders.py
- `test_basis_features_are_strictly_causal()` --calls--> `build_features()`  [INFERRED]
  tests/test_signal_layer.py → /Users/samihabbal/Documents/Claude/Projects/My personal Profile and who i am/cardamom-quant/src/features/engineering.py

## Communities (74 total, 9 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.1
Nodes (31): ablation_table(), block_bootstrap_sharpe(), deflated_sharpe(), expected_max_sharpe_annual(), probabilistic_sharpe(), Robustness statistics — is the Sharpe distinguishable from luck?  Three tools mo, results: {variant_name: metrics_dict} -> tidy comparison frame., Circular block bootstrap of the annualized Sharpe ratio. (+23 more)
...
Cohesion: 0.06
Nodes (36): build_physics_features(), _crop_year(), _day_in_crop_year(), hurst_rs(), inventory_overhang(), Auction-physics features — signal mined from columns the pipeline never touched,, Assemble the block on the market calendar; all columns lag-safe., R/S Hurst exponent estimate (three-scale log-log slope). (+28 more)
...
Cohesion: 0.05
Nodes (45): ABC, BaseLoader, BaseLoader, fetch(), parse(), BaseLoader: the common contract every real-data feed implements.  Design princip, Raised when a loader's output violates its schema contract., Fetch → parse → validate → load, with an immutable raw cache. (+37 more)

## Knowledge Gaps
- **238 isolated node(s):** `Mutating TODAY's auction must not change TODAY's features.`, `The synthetic extension must not contaminate real rows' features.`, `Mutating TODAY's auction must not change TODAY's features.`, `Real-shaped spot+futures frame (as build_market_dataset would join     them, pos`, `Mutating TODAY's basis (as build_market_dataset would compute it from     a chan` (+233 more)
...
- **Why does `build_features()` connect `Community 65` to `Community 0`, `Community 2`, `Community 4`, `Community 6`, `Community 7`, `Community 31`?**
  _High betweenness centrality (0.123) - this node is a cross-community bridge._
- **Why does `parse_oni()` connect `Community 3` to `Community 44`, `Community 31`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `main()` connect `Community 65` to `Community 0`, `Community 35`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Are the 20 inferred relationships involving `build_features()` (e.g. with `main()` and `run_cv()`) actually correct?**
  _`build_features()` has 20 INFERRED edges - model-reasoned connections that need verification._
- **Are the 16 inferred relationships involving `PurgedWalkForward` (e.g. with `main()` and `run_cv()`) actually correct?**
  _`PurgedWalkForward` has 16 INFERRED edges - model-reasoned connections that need verification._
